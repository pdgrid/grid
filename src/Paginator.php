...
class Paginator
{
    ...
    public function getRowPosition($where, $bind = null) {
        ...
        $select = $this->db->select()->from(['pdgrid_paginator' =>
            new \Zend_Db_Expr('(SELECT *, @rownum := @rownum + 1 AS pdgrid_paginator_pos FROM ('
                .$this->sql.
                ') pdgrid_paginator_t JOIN (SELECT @rownum := 0) pdgrid_paginator_c)')
        ], ['pdgrid_paginator_pos']);
